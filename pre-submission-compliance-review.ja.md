...
token revocation と local authorized-data deletion の改善は実装済みですが、最終監査提出前には次の対応がまだ必要です。
...
| programmatic token revocation | `POST /api/youtube/auth/logout` が local token clearing 前に Google token revoke endpoint を呼ぶ | 実装済み、screenshot/demo 証跡待ち |
| local YouTube Authorized Data deletion | `DELETE /api/youtube/authorized-data` が local YouTube SQLite rows と token を削除 | 実装済み、screenshot/demo 証跡待ち |
| Access Gate の分離 | Google ID token access gate は YouTube OAuth と別物として説明可能 | 補足資料として準備済み |

## 3. 必須または強く推奨する改善

### Issue 1: Google token の programmatic revocation 証跡撮影が必要
...
- backend はローカル保存された YouTube access / refresh token を消去する前に Google OAuth token revocation endpoint を呼びます。
- network error や server error で revocation に失敗した場合、再試行できるように local token を保持します。
...
- disconnect control と confirmation を示す app screenshot / demo video を撮影する。
- revocation success と明示的 failure handling を示す test output または log を用意する。
- Google consent screen の app name と scope が提出する OAuth 設定と一致していることを確認する。

対応後の証跡候補:

- token revoke request の integration test。
- アプリ内 disconnect の screenshot。
- token が revoke された、または明示的 error が返ることを示す test / log。

### Issue 2: Authorized Data deletion workflow の証跡撮影が必要

重大度: Major

現在の挙動:

- ユーザーは StreamNexus settings から local authorized-data deletion control を使用できます。
- backend は残っている token の revocation、local YouTube OAuth token clearing、SQLite の local YouTube rows 削除を行います。
- local deletion は `platform = 'youtube'` の `tracked_channels`、`stream_history`、`watch_sessions`、`stream_notes`、`claim_history` を対象にします。
- Privacy Policy は実際の削除挙動を説明し、YouTube account data は削除されないことを明記しています。
...
- Settings の local deletion control を撮影する。
- YouTube account、動画、YouTube 側の登録チャンネル本体は削除されないことを説明する confirmation text を撮影する。
- integration test と Privacy Policy の記載を補足証跡として保持する。
