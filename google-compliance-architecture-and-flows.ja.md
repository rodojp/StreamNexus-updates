...
- [アーキテクチャ図 PNG](./assets/google-compliance/streamnexus-architecture.png)
- [アーキテクチャ図 JPEG](./assets/google-compliance/streamnexus-architecture.jpg)
...
- [OAuth ユーザーフロー PNG](./assets/google-compliance/streamnexus-user-flow.png)
- [OAuth ユーザーフロー JPEG](./assets/google-compliance/streamnexus-user-flow.jpg)
...
- [連携解除・削除フロー PNG](./assets/google-compliance/streamnexus-revocation-deletion-flow.png)
- [連携解除・削除フロー JPEG](./assets/google-compliance/streamnexus-revocation-deletion-flow.jpg)
...
flowchart TD
  ...
  Logout --> Revoke["Google token revoke endpoint を呼び出す"]
  Revoke --> ClearTokens["ローカル保存された YouTube access / refresh token を消去"]
  ClearTokens --> AppState["アプリの認証状態を更新"]
  Settings --> DeleteLocal["local authorized-data deletion を押す"]
  DeleteLocal --> DeleteRoute["DELETE /api/youtube/authorized-data"]
  DeleteRoute --> DeleteRows["SQLite の local YouTube rows を削除"]
...
- アプリ内の連携解除では、ローカル保存された YouTube OAuth token を消去する前に Google token revocation を呼び出します。
- network error や server error で revocation に失敗した場合、再試行できるように local token を保持します。
- local authorized-data deletion action は、残っている token の revocation、local token clearing、SQLite の YouTube rows 削除を行います。
...
- 実装済みの revocation control と local deletion control を表示する screenshot / demo video を撮影する。
...
| アプリ内 revoke 導線 | `POST /api/youtube/auth/logout` が Google token revoke endpoint を呼ぶ | 実装済み、screenshot/demo 証跡待ち |
| Authorized data deletion | `DELETE /api/youtube/authorized-data` が local YouTube rows を削除 | 実装済み、screenshot/demo 証跡待ち |
